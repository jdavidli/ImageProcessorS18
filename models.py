...
def create_user(email, orig_img_paths, command, orig_timestamp):
    ...
    """
    u = models.User(email, orig_img_paths, command, orig_timestamp, [], [], [])  # create a new User instance
...
def add_images(email, img_paths, comms, timestamp):
    ...
    user.orig_img_paths.append(img_paths)
    user.command.append(comms)
    user.timestamp.append(timestamp)
